<template>
    <Master>
        <div class="container py-2">
            ...
            <div class="d-flex justify-content-center my-3" v-if="questions.links && questions.links.length > 3">
    <ul class="pagination pagination-circle shadow-sm p-1 bg-white border rounded-pill align-items-center">
        <li 
        ...
        >
            <Link 
                ...
                :style="link.label.includes('Previous') || link.label.includes('Next') 
            ...
            <div class="card border-0 shadow-sm rounded-3 mb-4 bg-white overflow-hidden" v-for="q in questions.data" :key="q.id">
                <div class="card-header bg-light border-0 py-3 px-4 d-flex align-items-center justify-content-between">
                    <div class="d-flex align-items-center gap-2">
                        <span :class="q.is_fixed ? 'badge bg-success-subtle text-success' : 'badge bg-danger-subtle text-danger'" class="rounded-pill px-2.5 py-1.5 fw-bold text-uppercase small" style="font-size:0.7rem;">
                            <i :class="q.is_fixed ? 'fas fa-check-circle' : 'fas fa-exclamation-circle'" class="me-1"></i> {{ q.is_fixed ? 'Fixed' : 'Need Fix' }}
                        </span>
                        <span class="text-dark fw-bold text-truncate ms-1" style="max-width:300px;">{{ q.slug }}</span>
                    </div>
                    <!-- Owner Actions -->
                    <div class="d-flex gap-2" v-show="isOwner(q.user_id)">
                        <Link href="" class="btn btn-sm btn-link text-warning p-0 fw-bold text-decoration-none" style="font-size:0.8rem;"><i class="fas fa-tools"></i> Fix?</Link>
                        <button @click="LandC.deleteQues(q.id)" class="btn btn-sm btn-link text-danger p-0 fw-bold text-decoration-none" style="font-size:0.8rem;"><i class="fas fa-trash-alt"></i> Delete?</button>
                    </div>
                </div>
                ...
                <div class="card-footer bg-white border-top border-light px-4 py-3">
                    <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3">
                        ...
                        <div class="d-flex flex-wrap gap-1.5">
                            <Link href="" v-for="t in q.tag" :key="t.id" class="badge rounded-pill bg-light text-secondary border px-2.5 py-1.5 font-weight-medium text-decoration-none hover-tag" style="font-size:0.75rem;">
                                # {{ t.name }}
                            </Link>
                        </div>
                        <div>
                            <Link :href="route('question.details', q.slug)" class="btn btn-sm btn-primary px-3 py-1.5 fw-bold text-capitalize shadow-none" style="border-radius:6px;">View Details</Link>
                        </div>
                    </div>

                    <!-- 3. CLEAN COMMENT BOX PANEL -->
                    <div class="mt-4 pt-3 border-top border-light fade-in" v-if="LandC.showCommentBox.value === q.id">
                        <form @submit.prevent="LandC.addComment(q)">
                            <div class="input-group shadow-sm rounded-3 overflow-hidden border bg-white p-1 mb-4">
                                <input type="text" class="form-control border-0 py-2 shadow-none text-dark ps-3" v-model="LandC.comment.value" placeholder="Share your technical solution..." style="font-size:0.9rem;" />
                        ...
                        <div class="d-flex flex-column gap-3 mb-2">
                            <div class="bg-light rounded-3 p-3 border" v-for="com in q.comment" :key="com.id">
                                <div class="d-flex align-items-center justify-content-between mb-2">
                                    <div class="d-flex align-items-center gap-2">
                                        <img :src="q.user.image ? '/profile/' + q.user.image : '/images/default-avatar.png'" class="rounded-circle border" style="width:26px; height:26px; object-fit:cover;">
                                        <span class="fw-bold text-dark" style="font-size:0.85rem;">{{ q.user.name }}</span>
                                    </div>
                                    <span class="text-muted small" style="font-size:0.75rem;"><i class="far fa-clock me-1"></i>{{ com.date }}</span>
                                </div>
